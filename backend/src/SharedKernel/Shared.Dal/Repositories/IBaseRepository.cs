...
public interface IBaseRepository
{
    ...
    //ValueTask<PaginatedResponse<TEntity>> GetListAsync<TEntity>(PaginationFilter filter) where TEntity : Entity;
    //ValueTask<PaginatedResponse<TEntity>> GetEntitiesByConditionAsync<TEntity>(Expression<Func<TEntity, bool>> condition, PaginationFilter filter) where TEntity : Entity;
...
public class BaseRepository<TContext> : IBaseRepository where TContext : DbContext
{
    ...
    //public async ValueTask<PaginatedResponse<TEntity>> GetListAsync<TEntity>(PaginationFilter filter) where TEntity : Entity
    //    => await _context.Set<TEntity>().AsNoTracking().PaginateAsync(filter);

    //public async ValueTask<PaginatedResponse<TEntity>> GetEntitiesByConditionAsync<TEntity>(Expression<Func<TEntity, bool>> condition, PaginationFilter filter) where TEntity : Entity
